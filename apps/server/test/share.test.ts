...
describe("分享連結", () => {
  ...
  async function makeShare(vaultId: string, docId: string, permission: "read" | "write"): Promise<{ owner: Client; shareId: string }> {
    ...
    // 先塞一筆內容讓 doc 存在
    owner.send({ type: "push", docId, deviceId: "owner", counter: 1, epoch: 0, authorMemberId: "", sig: new Uint8Array(), payload: new Uint8Array([1, 2, 3]) });
  ...
  it("唯讀分享:收件人可拉既有內容,推送被拒", async () => {
    ...
    guest.send({ type: "push", docId: "doc-唯讀", deviceId: "guest", counter: 1, epoch: 0, authorMemberId: "", sig: new Uint8Array(), payload: new Uint8Array([9]) });
  ...
  it("唯讀分享:snapshotPush 也被拒,不得覆寫快照或截斷增量", async () => {
    ...
    guest.send({ type: "snapshotPush", docId: "doc-唯讀快照", uptoSeq: 1, epoch: 0, authorMemberId: "", sig: new Uint8Array(), payload: new Uint8Array([0, 0]) });
  ...
  it("可編輯分享:收件人推送被接受,owner 收到廣播", async () => {
    ...
    guest.send({ type: "push", docId: "doc-可編輯", deviceId: "guest", counter: 1, epoch: 0, authorMemberId: "", sig: new Uint8Array(), payload: new Uint8Array([42]) });
  ...
  it("廣播按作用域過濾:分享連線只收得到自己那篇", async () => {
    ...
    owner.send({ type: "push", docId: "分享篇", deviceId: "owner", counter: 1, epoch: 0, authorMemberId: "", sig: new Uint8Array(), payload: new Uint8Array([1]) });
    await owner.next("ack");
    ...
    owner.send({ type: "push", docId: "別篇", deviceId: "owner", counter: 1, epoch: 0, authorMemberId: "", sig: new Uint8Array(), payload: new Uint8Array([2]) });
    await owner.next("ack");
    // owner 改動「分享篇」——guest 應收到
    owner.send({ type: "push", docId: "分享篇", deviceId: "owner", counter: 2, epoch: 0, authorMemberId: "", sig: new Uint8Array(), payload: new Uint8Array([3]) });
    await owner.next("ack");
  ...
  it("撤銷後分享失效,收件人認證被拒", async () => {
    ...
    owner.send({ type: "push", docId: "doc-撤銷", deviceId: "owner", counter: 1, epoch: 0, authorMemberId: "", sig: new Uint8Array(), payload: new Uint8Array([1]) });
    await owner.next("ack");
  ...
  it("撤銷立即踢掉既有連線,撤銷後的內容一個字都拿不到", async () => {
    ...
    owner.send({ type: "push", docId: "doc-撤銷即時", deviceId: "owner", counter: 2, epoch: 0, authorMemberId: "", sig: new Uint8Array(), payload: new Uint8Array([7]) });
    await owner.next("ack");
  ...
  it("撤銷只踢該分享的連線,同 vault 的其他連線不受影響", async () => {
    ...
    // 另一則分享仍活著:續寫仍收得到
    owner.send({ type: "push", docId: "doc-撤銷隔離", deviceId: "owner", counter: 2, epoch: 0, authorMemberId: "", sig: new Uint8Array(), payload: new Uint8Array([5]) });
    await owner.next("ack");
  ...
  it("撤銷不跨 vault:猜中他人 shareId 也踢不掉對方的連線", async () => {
    ...
    victimOwner.send({ type: "push", docId: "doc-受害", deviceId: "owner", counter: 2, epoch: 0, authorMemberId: "", sig: new Uint8Array(), payload: new Uint8Array([6]) });
  ...
  it("shareList 列出本 vault 全部分享,異 vault 看不到", async () => {
    ...
    owner.send({ type: "push", docId: "d1", deviceId: "o", counter: 1, epoch: 0, authorMemberId: "", sig: new Uint8Array(), payload: new Uint8Array([1]) });
    await owner.next("ack");
